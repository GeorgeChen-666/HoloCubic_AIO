...
static lv_obj_t *income_gui = NULL;
...
LV_IMG_DECLARE(income_logo_ico);
...
void display_income_init(void)
{
    lv_obj_t *act_obj = lv_scr_act(); // 获取当前活动页
    if (act_obj == income_gui)
        return;

    income_obj_del();    // 清空对象
    lv_obj_clean(act_obj); // 清空此前页面

    income_gui = lv_obj_create(NULL, NULL);
    fans_label = lv_label_create(income_gui, NULL);
    follow_label = lv_label_create(income_gui, NULL);
    logo_image = lv_img_create(income_gui, NULL);
    avatar_image = lv_img_create(income_gui, NULL);
...
void display_income(const char *file_name, lv_scr_load_anim_t anim_type,
                      const char *fans_num, const char *follow_num)
{
    display_income_init();
    lv_obj_add_style(income_gui, LV_BTN_PART_MAIN, &default_style);
    ...
    lv_img_set_src(logo_image, &income_logo_ico);
    lv_obj_align(logo_image, NULL, LV_ALIGN_IN_TOP_MID, 0, 20);
    lv_obj_set_size(logo_image, 200, 62);

    lv_img_set_src(avatar_image, "S:/income/avatar.bin");
