#include "income.h"
#include "income_gui.h"
#include "sys/app_controller.h"
#include "../../common.h"

#define FANS_API "https://api.airtable.com/v0/appUDWxX2DaUTgZvp/%E8%B4%A6%E6%9C%AC?fields%5B%5D=%E9%87%91%E9%A2%9D&view=%E5%BD%93%E6%9C%88%E7%B1%BB%E5%88%AB%E6%B1%87%E6%80%BB%E8%A1%A8"
#define OTHER_API "https://api.income.com/x/space/upstat?mid="

// 天气的持久化配置
#define B_CONFIG_PATH "/income.cfg"
struct B_Config
{
    String bili_uid;              // income的uid
...
static MyHttpResult http_request(String addr)
{
    ...
    httpClient->addHeader("Authorization", "Bearer patdXVn252UGkfOA3.255e1e7c72affbf2924f843f7d27b471c27cbbaa1c9723d0aa4ee90e1cc43177");
...
static int income_init(AppController *sys)
{
    income_gui_init();
    // 获取配置信息
...
static void income_process(AppController *sys,
                             const ImuAction *act_info)
{
    ...
    {
        display_income("income", anim_type, fans_num, follow_num);
        // 以下减少网络请求的压力
...
static void income_background_task(AppController *sys,
                                     const ImuAction *act_info)
{
...
static int income_exit_callback(void *param)
{
    income_gui_del();
...
{
    MyHttpResult result = http_request(FANS_API);//cfg_data.bili_uid
...
static void income_message_handle(const char *from, const char *to,
                                    ...
                                    void *ext_info)
{
    ...
    {
        Serial.print(millis());
        Serial.println("[SYS] income_event_notification");
        if (0 == run_data->refresh_status)
        {
            update_fans_num();
        }
...
APP_OBJ income_app = {BILI_APP_NAME, &app_income, "", income_init,
                        income_process, income_background_task,
                        income_exit_callback, income_message_handle};
